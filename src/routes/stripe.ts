...
export async function webhookHandler(req: Request, res: Response) {
  ...
  if (!sig || !secret) {
    return res
      .status(400)
      .json({ error: { code: 'WEBHOOK', message: 'Missing signature/secret' } });
  }
  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'invoice.payment_succeeded':
        await recordPayment(client, event.data.object as Stripe.Invoice, 'succeeded');
        break;
      case 'invoice.payment_failed':
        await recordPayment(client, event.data.object as Stripe.Invoice, 'failed');
        break;
      default:
...
async function handleSubscriptionUpdated(
  ...
  const userIdRaw =
    ...
    (await lookupUserIdByStripeCustomer(client, subscription.customer as string));
  if (!userIdRaw) return;
  const userId = Number(userIdRaw);
  const priceId = subscription.items.data[0]?.price.id ?? '';
  const plan = (subscription.metadata?.plan as Plan | undefined) ?? planForPriceId(priceId);
  if (!plan) return;
  await upsertSubscription(client, userId, plan, subscription);
  await applyTierFromSubscription(client, userId, plan, subscription.status);
}
